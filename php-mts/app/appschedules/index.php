...
<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">Appointment Schedule List</h3>
        <center>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            serverSide: true,
            ajax: {
                url:"../classes/Master.php?f=dt_appschedules",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.department_name))+' ...</p>';
                        },
                    width:"20%"
                },
                {
                    className: 'py-1 px-2',
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.time))+'</p>';
                        },
                    width:"5%"
                },
                {
                    className: 'py-1 px-2',
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.hospital))+'</p>';
                        },
                    width:"5%"
                },
                {
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.find('.edit_data').attr("href","./?page=appschedules/manage_schedule&id="+row.id)
                        el.find('.view_data').attr("href","./?page=appschedules/view_details&id="+row.id)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Are you sure to delete <b>"+$(this).attr('data-name')+"</b> from list?","delete_appschedule",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
    ...
    function delete_appschedule($id){
        start_loader();
        var _this = $(this)
        $('.err-msg').remove();
        var el = $('<div>')
